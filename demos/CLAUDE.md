...
| `dna-lab.html` | Base pairing, then the nucleotide. Step 1: drag two bases together and the hydrogen bonds form only where the declared Watson–Crick edges allow, solved by `dna/pairing.js`. Step 2: drag deoxyribose and phosphate onto a base and watch two condensations build a nucleotide, in poses `dna/attach.js` reads out of 1BNA. Step 3: a second rung under the first, joined by two phosphodiester bonds that run opposite ways — antiparallel, read off the record rather than arranged. Step 4: the ladder twisting into the helix | prototype |
