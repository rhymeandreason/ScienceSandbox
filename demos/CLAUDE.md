...
| `hemoglobin/tools/check-hb.js` | the 68 assertions behind the haemoglobin page: staleness, quantisation, the two decoders agreeing, DSSP vs the deposited HELIX records, the opening close-up (its atoms are the ribbon's, and its helix is made before the camera leaves it), and **helix handedness**, which is the one global mirror an internal check can actually catch | own header |
