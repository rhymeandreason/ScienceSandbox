...
| What scale a component is, whether two can share a scene, or what size a page may print | `Scale.md` — the nine-rung ladder, `form`, and why most renders declare no unit at all. `kit/scale.js` is the enum and `tools/check-scale.js` the enforcement |
...
| `build/build.html` | The builder: a request becomes an app from `Components.md`, stored in the database with every edit a version, run in a sandboxed frame. `app.html` beside it is the viewer. `Generator.md` | internal tool |
...
| `dna-lab.html` | Base pairing, then the nucleotide. Step 1: drag two bases together and the hydrogen bonds form only where the declared Watson–Crick edges allow, solved by `dna/pairing.js`. Step 2: drag deoxyribose and phosphate onto a base and watch two condensations build a nucleotide, in poses `dna/attach.js` reads out of 1BNA. Step 3: two more rungs, one below and one above, each joined by two phosphodiester bonds; the two rails run opposite ways — antiparallel, read off the record rather than arranged. The rungs arrive unwound and the twist is what closes the bonds. Step 4 is open: the winding animation that was there lives whole in `tests/ladder-helix-test.html` | prototype |
