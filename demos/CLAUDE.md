...
| `glycolysis-lab.html` | All ten steps, grouped into five stages: carbon bookkeeping (why it costs 2 ATP to make ATP), PFK-1 as the regulated committed step, the pathway's one oxidation, and which three steps are irreversible — plus the **ATP carrier drawn as a molecule** on the stage — one object losing and regaining its γ phosphate, with a schematic fallback behind the "Show full molecules" checkbox — and a **mass-action modal** off the reversibility note, which is a second simulation with its own physics (see below) | pathway | prototype |
