...
| `dna-lab.html` | Base pairing, then the nucleotide. Step 1: drag two bases together and the hydrogen bonds form only where the declared Watson–Crick edges allow, solved by `dna/pairing.js`. Step 2: drag deoxyribose and phosphate onto a base and watch two condensations build a nucleotide, in poses `dna/attach.js` reads out of 1BNA. Step 3: the third condensation, one nucleotide's 3′–OH onto the next one's phosphate, ending on a strand with a 5′ end and a 3′ end. Step 4: the ladder twisting into the helix | prototype |
