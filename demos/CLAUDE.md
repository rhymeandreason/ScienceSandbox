...
| `chain/glucose-chains-test.html` | Test bench for `chain-repeat.js` — one linkage, measured off the disaccharide, repeated twelve times. Cellulose's two-fold ribbon and starch's six-fold helix both fall out of the torsions, with nothing drawing either. A third chain (galactan) varies the acceptor's C4 instead of the donor's C1 | test |
