...
| `hemoglobin-lab.html` | **Level 4 is why this page exists.** One β chain of haemoglobin (PDB 2HHB, X-ray 1.74 Å) folds from a 503 Å extended chain to the 45 Å globin fold. `folding-lab-ribbon` can only reach level 3 — villin is one chain of 36 residues — and haemoglobin is the molecule AP Bio actually uses to teach quaternary structure. **The two mechanisms separate visibly here in a way villin's do not**: between t=0.59 and t=0.85 the H-bond count is flat while the molecule halves in size, so the page shows both numbers side by side and greys out whichever level is not currently working. Ribbon-only — at 146 residues a backbone is 729 atoms and reads as spaghetti, so the cartoon is the only legible representation rather than an overlay on one. **The helix is drawn from the H-bonds that have formed, not statically**, or level 2 would look finished at t=0. Its "finished shape" toggle ghosts in the deposited chain, which is the honest way to show that the fold is steered toward a known answer, and its
**last frame IS that answer** — `land()` in the baker eases the chain onto the
deposited coordinates over the final 14%, because a 1.24 Å relaxation draws as a
kinked cartoon even though the number sounds fine. Two traps are recorded there:
...
| `hemoglobin-lab` | palette, molecules, scene, folding/ribbon, hemoglobin/hbfold |
...
| `hemoglobin/tools/bake-hb.js` | folds 2HHB chain B once (8.5 s, 1.24 Å RMSD) and writes `hemoglobin/data/2HHB-B.fold.bin` (403 KB): 146 Cα + 103 O + 103 H per keyframe, int16. **Re-run after any change to `folding/folding.js`'s solver** — `hemoglobin/tools/check-hb.js` re-bakes and compares | own header |
| `hemoglobin/tools/check-hb.js` | the 27 assertions behind the haemoglobin page: staleness, quantisation, the two decoders agreeing, DSSP vs the deposited HELIX records, and **helix handedness**, which is the one global mirror an internal check can actually catch | own header |
