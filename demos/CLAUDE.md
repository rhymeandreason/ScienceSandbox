...
**`protein-lab.html` is not coming back — `hemoglobin-lab.html` supersedes it**, carrying all four levels on one molecule where the old page needed two structures and a swap between them. Treat the four levels as covered; there is no rewrite pending.

`pdb.js` (with `pdb/*.pdb` and `tools/check-pdb.js`) stays, and **not** as dead code kept for sentiment: `molstar/protein-inhouse.html` and `molstar/protein-molstar.html` both load it and call `PDBLib.orient()` — the second calls `PDBLib.helices()` too. No *lesson* loads it, which is why it is absent from the per-page table above, and that is exactly what makes it easy to delete by accident. `tools/check-pdb.js` is the only test of its det = +1 mirror guard, the failure MolecularGeometry.md §1.3 says internal checks are otherwise blind to; `check-folding.js` guards `folding/folding.js`'s separate copy of the same code, not this one.
...
| `folding/ribbon.js` | `RibbonLib` = `build` (Cα trace + secondary structure → a ribbon `BufferGeometry`) + `dssp`/`parseBackbone` (Kabsch & Sander, the real thing, needs N/CA/C/O) + `assign`/`detect` + `HP35_HELICES`. `hemoglobin-lab.html` (the lesson), plus `folding-lab-ribbon.html` and `bake-villin.js`. Real ångströms, no materials — the page owns those. **Called per frame on live fold coordinates**, so a ribbon is only ever as trustworthy as the solver's frame under it | own header |
| `hemoglobin/hbfold.js` | `HbFold` = `decode` (the baked hemoglobin fold → Cα trace, secondary structure, H-bond list, the residue sequence, the **focus segment**'s full backbone atoms, `at(t)`). `hemoglobin-lab.html` only. Real ångströms, no THREE — it returns plain arrays | own header |
