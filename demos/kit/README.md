...
| `inset.js` | **one molecule in a box, over a scene at another scale.** The framed close-up a figure uses when the stage draws the same object schematically — membrane-lab's bilayer is instanced cylinders, which says nothing about what a lipid IS. Owns its own renderer, a camera solved from the molecule's extent, an optional turntable that moves the CAMERA (so a spec's `view:` is still what the box opens on and the group's rotation stays identity), and an IntersectionObserver that stops the loop when nobody is looking. One context, reused via `show()`; `destroy()` releases it. The frame lives in `main.css`'s `.inset` and is not decoration — unframed, a magnified molecule reads as an object standing in the scene at that size | the human, `membrane-lab` |
