...
import { CATS, SHARES, PCT } from './piles.js';

/* ---------- formatting helpers ---------- */
const fmtInt = n => Math.round(n).toLocaleString('en-US');
const lbOz = lb => {
  const oz = Math.round(lb * 16), l = Math.floor(oz / 16), o = oz % 16;
...
/* ---------- forestry model (step 6) ---------- */
const heightFor = D => 1.3 + 30 * (1 - Math.exp(-D / 40));               // m, saturating
const dryMassFor = D => 0.0673 * Math.pow(0.6 * D * D * heightFor(D), 0.976); // kg, Chave et al. 2014
const REF_D = 60;
...
export const steps = [
  /* 1 ------------------------------------------------------------ */
  {
    ...
    onEnter(ctx) {
      const { ui, state } = ctx;
      ctx.showPotScene(false);
      ctx.setDaylight(1);
      ctx.flowsOff();
      ctx.piles.gather();
      ctx.setTreeOpacity(1);

      ui.controls(`
        <div class="choices" role="group" aria-label="Your guess">
          ${['The soil', 'Water', 'The air', 'Sunlight'].map(c => `<button class="choice" data-choice="${c}">${c}</button>`).join('')}
        </div>
  ...
  {
    id: 'helmont',
    eyebrow: 'An experiment, 1640s',
    title: 'Van Helmont weighs a willow.',
    ...
    onEnter(ctx) {
      ...
      ui.controls(`
        ...
        <button class="btn secondary is-hidden" id="weigh">Weigh the soil again</button>
      ...
      const set = y => {
        state.year = y;
        const f = y / 5;
        const treeLb = 5 + 164.19 * Math.pow(f, 1.4);
        const soilLb = 200 - 0.125 * f;
        ui.q('#yr-val').textContent = y.toFixed(1);
        ui.q('#tree-lb').textContent = lbOz(treeLb);
        ui.q('#tree-kg').textContent = kgFrom(treeLb);
        ui.q('#soil-lb').textContent = lbOz(soilLb);
        ui.q('#soil-kg').textContent = kgFrom(soilLb, 2);
      ...
      ui.range(ui.q('#yr'), set);
      weigh.addEventListener('click', () => {
        ui.hide(weigh);
        ui.show(note);
        ui.setNext('So where, then?');
      });
    },
  },
  ...
  {
    id: 'water',
    eyebrow: 'A reasonable mistake',
    title: 'His answer: water.',
    ...
    onEnter(ctx) {
      const { ui } = ctx;
      ctx.showPotScene(true);
      ctx.sapling.setGrowth(1);
      ui.controls(`
        <button class="btn secondary" id="show-air">Look at the air</button>
      ...
      ui.q('#show-air').addEventListener('click', () => {
        ...
        ui.hide(ui.q('#show-air'));
        ui.show(ui.q('#air-note'));
        ui.setNext('How it gets in');
      });
    },
  ...
  /* 4 ------------------------------------------------------------ */
  {
    ...
    title: 'The tree is built from air.',
    ...
    onEnter(ctx) {
      ...
      ui.controls(`
        ...
        <div class="chips">
          <button class="chip" data-flow="co2"><i style="--c:var(--air)"></i>CO₂ in</button>
          <button class="chip" data-flow="h2o"><i style="--c:var(--water)"></i>Water up</button>
          <button class="chip" data-flow="o2"><i style="--c:var(--o2)"></i>O₂ out</button>
        </div>
  ...
  /* 5 ------------------------------------------------------------ */
  {
    ...
    onEnter(ctx) {
      ...
      ui.controls(`
        <button class="btn secondary" id="apart">Take it apart</button>
        <div class="seg is-hidden" id="mode" role="group" aria-label="Weigh as">
          <button data-mode="dry" class="is-on">Dry wood</button><button data-mode="fresh">Living tree</button>
        </div>
      ...
      ui.q('#apart').addEventListener('click', () => {
        ui.hide(ui.q('#apart'));
        ctx.setTreeOpacity(0.12, 1.2);
        setMode('dry');
        ui.show(ui.q('#legend'));
        ui.show(ui.q('#mode'));
        ui.show(ui.q('#apart-note'));
  ...
  /* 6 ------------------------------------------------------------ */
  {
    id: 'scale',
    eyebrow: 'Scale it up',
    title: 'How much air is in a tree?',
  ...
  /* 7 ------------------------------------------------------------ */
  {
    ...
    onEnter(ctx) {
      ...
      ctx.setTreeOpacity(1);
      ctx.piles.gather();
      flows.co2.setIntensity(0.8);
      flows.o2.setIntensity(0.6);
      flows.h2o.setIntensity(0.6);
      flows.minerals.setIntensity(0.6);
      ui.controls(`
        <div class="legend">${legendHtml('dry')}</div>
        <p class="foot">Shares of dry mass. Numbers are rounded; real trees vary by species.</p>
        <button class="btn ghost" id="restart" style="align-self:flex-start">Start again</button>`);
