 ...
 *
 * ---------------------------------------------------------------------
 *  THERE IS NO PAGE ANY MORE. sickle-lab.html was deleted, and with it
 *  went a whole section of this file: that the page loaded the right
 *  modules and no mol-*.js, that it mutated BOTH beta chains, that it
 *  drew no InstancedMesh dot-cloud, that two specific wrong phrasings
 *  had not come back, and that every number in the prose carried a
 *  data-num attribute instead of being typed.
 *
 *  Those are gone rather than commented out, because an assertion about
 *  a file that does not exist is not a weakened check, it is a check
 *  that cannot fail. What is left is everything that was ever about the
 *  DATA — which is most of it, and all of the three risks below.
 *
 *  IF A PAGE COMES BACK, the page-side assertions have to come back with
 *  it. The two worth restoring first are the ones that caught real bugs:
 *  BOTH beta chains carry the mutation (an early build grafted onto one,
 *  so half the sites did not change when the student toggled), and no
 *  figure is typed into the prose (a typed number is a claim nothing
 *  checks and that a re-bake silently makes false). Neither is
 *  reconstructible from this file alone — that is why they are described
