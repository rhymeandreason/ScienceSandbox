...
**A duplicate is disambiguated by the field it is written under, not refused for being one.** A step's `nextLabel` names the step after it and that step's `eyebrow` repeats the words, so neither is unique on its own and both are unique with their key: the find widens to `eyebrow: 'Hypotonic Solution'` and the replacement puts the key back. `app-edit.js`'s `KEYS` maps the shell's class names to the five fields that hold a page's words, which is the only thing that can tell two identical passages apart. Two passages under the SAME key are still ambiguous and still refused. Across the eval set this is every duplicate there is: 11 of 217 short fields were blocked, and 11 are reachable.
