...
| `diagram-2d.js` | `Diagram2D` = `draw(el,spec,opts)` · `highlight(spec,refs)` · `mode(spec)` + the ink/box constants. The flat drawing under a 3D model: picks Haworth for a sugar, a LEWIS structure for a molecule too small to have a skeleton (its layout is topological, never the spec's coordinates), and SmilesDrawer for everything else, sharing one palette and one font across all three. Bench: `tests/diagram-2d-test.html`. Does NOT read `spec.flat2d` (that is bake-flat2d.js's RDKit layout, for `molecule-viewer.html`). The page loads SmilesDrawer, not the module | own header |
