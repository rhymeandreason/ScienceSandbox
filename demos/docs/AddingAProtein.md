...
* **`does` says what the protein is FOR**, in one word from a vocabulary — `enzyme`, `oxygen carrier`, `unknown` — because it is the first question a reader has and the one a ribbon cannot answer. Free text would split the collection in two on a filter the day someone wrote "an enzyme". **`unknown` is an answer**: PrP's healthy function has been argued for forty years, and a registry that guessed would be teaching one side of it.
