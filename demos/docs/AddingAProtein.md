...
* **A protein whose files another pipeline writes gets `pipeline:'own'`.** Haemoglobin is the case: `hemoglobin/tools/` bakes a trace, a quaternary file, a surface and an 830 KB fold for the folding lesson, and nothing here should reach into that. Its `read` block is written by `proteins/tools/read-own.js` from the DEPOSITION each variant names in `source.path`, `bake:` says which of that folder's files the entry means, and `page:` points at the lesson where every other protein derives a bench. The checker verifies the same four fields against the same deposition and leaves that folder's files alone.
