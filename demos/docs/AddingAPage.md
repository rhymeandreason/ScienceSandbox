...
* **Every spec in `lib/` is one scale, so two of them share a scene safely.** What still does not is a protein: deposited coordinates are real ångströms drawn at `PALETTE.radii / SCALE`, computed in the page. `register()` throws when two files define the same key — a separate rule, and the loser is overwritten in every scene at once. MolecularGeometry.md §1.5.
* **`mol-*.js` coordinates are real ångströms.** (`units:'scene'` is left for specs derived from an already-scaled one.) Pasting display-scale numbers into an `angstrom` spec is a silent 1.9× that reads as a styling choice. `tools/sdf2spec*.js` emit ångströms, so their output pastes in.
...
* **`mol-carriers.js` holds controls *and* the molecules three lessons draw.** `atpSkel` and `nadhSkel` are the same molecules as `atp` and `nadh`, built the other way, and they earn their place by matching the SAME `check-handedness.js` reference — that is what makes a visible difference between them method rather than a mistake. That reference is **derived from `compare:{against:…}`**, not typed: a control added without one would leave the tool reporting every spec passing while never having looked at the new one. A control whose partner has no reference fails the run. The pair is deliberate: NADH is twice the molecule, so it shows the schematic's cost GROWING with size (1.01 Å out of plane and 21.4 Å across, against the conformer's 1.91 and 12.0). They used to live in a `mol-compare.js` of their own, on the argument that a page loading the pathway file must not pay for a spec it never draws. That inverted: **glycolysis-lab, krebs-lab and fermentation-lab all draw their carriers from the schematics** (`ATP = M.atpSkel`), because a real conformer folds its tail over the γ phosphate and over the nicotinamide C4 — the exact atoms glycolysis steps 1/3/6/7/10 are about. Three lesson pages were loading a second file to reach their own carriers, so the file was dissolved into `mol-carriers.js`. Editing a "control" here changes three lessons, and no checker says so.
...
2. Copy the head (icons + `css/kodo.css`, plus `css/lesson-shell.css` if it takes that shell — `Design.md`'s sheet table — and the scripts you need). **Load only the `mol-*.js` domains your page shows**, after `molecules.js` (and after `skel.js` if any needs the builder).
...
4. `const {scene,camera,renderer,root,cam,applyCam,resize}=Stage.create(canvas,{...});` then `const FXi=FX.create(THREE,root,camera);` — skip FX only if the page fires nothing (`contrast-lab.html`).
