...
| **B. derived** | everything in `mol-aminoacids.js`, `mol-monomers.js`, `mol-nucleic.js`, `mol-pathways.js`, `mol-sugars.js`, `mol-glycans.js`, `mol-compare.js` — the amino acids, the four bases and the four dNTPs, AMP, glucose + all glycolysis intermediates, the mono- and disaccharides | **stored in real Å** (`units:'angstrom'`); `register()` applies the display scale once | **1.9×**, relative lengths truthful |
