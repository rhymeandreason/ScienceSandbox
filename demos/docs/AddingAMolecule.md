...
Skel, `units:'angstrom'`, and `register()` applies the display scale once. Grow C–H **last** so every index the spec refers to stays stable — glucose's discipline, and every spec since has kept it.
