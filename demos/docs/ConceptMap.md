...
**The surface is the CARD tier, never the lesson's.** Hemoglobin's card bake is 362 KB against 1.5 MB for the same structure at 0.7 A, and a 280 px thumb cannot show the difference — `tools/bake-card-surface.js` exists for exactly this. The path is derived from the trace's own stem (`2HHB.trace.json` → `2HHB.card.surf.bin`), so the two are the same structure by construction. It is offered ONLY where `surface.bake` is true in the registry, which is a judgement about whether a skin teaches anything: for the pump it hides the site that is the whole point, and for four of the six the registry says so in its own words. Amylase's card surface was baked to match hemoglobin's, since `bake: true` should mean the card can show one.
