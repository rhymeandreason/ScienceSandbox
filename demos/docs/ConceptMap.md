...
**Its path is NAMED, not derived.** The variant's `bake` block lists every artefact by role — `trace`, `quaternary`, `surface`, `card`, `fold` — and its own comment says why: which file is which is a decision, not something a filename proves, and `proteins/check-proteins.js` fails a role whose name is not on disk. A page that reconstructed `2HHB.card.surf.bin` from the trace's stem would be standing a convention where a fact already is, and it would go on working right up until one file was named differently. Read `v.bake.card`; no role, no toggle.

That also means the toggle follows the registry's judgement for free. Four of these six have no `card` role because a skin teaches nothing about them — for the pump it hides the site that is the whole point of the E1/E2 pair, and the entry says so. Amylase's card surface was baked and RECORDED as a role, since a file in `data/` that the registry does not name is exactly what `check-proteins.js` exists to catch.
