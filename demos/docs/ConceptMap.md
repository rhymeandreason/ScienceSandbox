...
16. **Every control a card carries must be named in the canvas's `pointerdown` escape list.** `canvas.addEventListener('pointerdown', ...)` captures the pointer for pan and drag, and bails out early only for a hard-coded selector (`.pbox-rep, .pbox-play, .lesson, .playhit, .mb-dims`). A control not in that list never sees its `pointerup`, so the click that reaches it is the card's: the button renders perfectly, highlights on hover, and does nothing while the card behind it opens. Nothing links a new control to that line, and `.playhit` shipped broken for exactly this reason. **A synthetic `click` in the console does not catch it** — it skips the pointer sequence the bug lives in, so it passes on a completely dead button. Test controls with a real click.
...
**The player chrome is ours too, as far as it goes.** `controls=0` takes YouTube's bar away and does not finish the job: the title, avatar, logo and share button still fade in over the picture. So the frame is `pointer-events: none` and the strip under it (play/pause, elapsed, scrubber, fullscreen) is the page's own, with the whole picture as a play/pause target. The end screen is killed by rewinding to 0 on `ENDED`, which puts frame 0 back and never lets the thumbnail grid draw.

**What is NOT solved, and the measurement that settled it.** YouTube draws its own overlay — big glyph, title, avatar, "More videos", logo — on every pause AND every seek, including seeks from a cue click. It does not fade on a timer worth waiting out: measured after a resume, it was still up at +0.5s, still up at +2s, and gone by +8s. Covering it would mean blacking out several seconds of the film every time the reader touches a cue, which costs more than the chrome does. **So the cover is the pre-play state only** — it hides the player's start-up, where nothing behind it is worth seeing, and once the picture has moved it never returns. A pause shows YouTube's own UI, which is what a paused embed looks like. The centre play mark is tied to the cover for the same reason: after start-up YouTube draws its own glyph, and two play marks over one frame is worse than either. Removing this properly means hosting the file, which needs the author's permission.

**Three traps in the player wiring.** `BUFFERING` is not `PAUSED`: YouTube fires it right after `PLAYING` and again on every seek, and treating it as a stop threw the poster over a running video for as long as the buffer took. `destroy()` takes the iframe with it, so only `.mstage` is rebuilt on close, never `.mvideo`, whose strip has live listeners bound to its nodes. And the API stamps `width="640" height="360"` **attributes** on the iframe it builds, so `#videoframe` needs an explicit `height: auto` or a presentational height beats `aspect-ratio` and the player comes out 1030x356 in a box that wanted 1030x579.
