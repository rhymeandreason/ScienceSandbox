...
* **`tests/cards-cluster.html`** — the stage bench: a wall of cards across every kind, against a pool budget of 4.
...
**The cue file is authored, and it carries two tracks.** YouTube publishes no caption text a page can read: the Data API's `captions.download` is owner-only OAuth, `timedtext` serves an empty body to a plain request, and the only track on the glycolysis video is auto-generated ASR, which renders enzyme names as noise. The shape is `{ tracks: [{ id, label, source, sourceUrl, cues: [{ t, text, step }] }] }`.
...
**The wiring traps are commented at their sites** in `question-composer.html` rather than here, since that is where they bite: `BUFFERING` is not `PAUSED`, `destroy()` takes the iframe so only `.mstage` is rebuilt on close, the API's `height` attribute beats `aspect-ratio`, and the cue column is absolutely positioned so it cannot size the card.
...
* **The absolute floor.** `MATCH = 0.85` was calibrated against the corpus as it stood. As the corpus densifies the nearest neighbour gets closer for EVERY query, off-map ones included, so the floor drifts without anything being wrong. It has to become relative, or be recalibrated at every significant growth. The fixture is what would catch that.
