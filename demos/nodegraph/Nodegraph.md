...
**And the graph arrives in order**, nearest the seed first: by hop, then left to right, which is the order the reader would have clicked them in. A wave that lands at once is a picture appearing; one that lands in reading order is the graph being built, which is the whole claim of the mode. It is a class and a per-element `transition-delay`, 70ms a card to a 900ms cap, and an EDGE waits for its later end or a line reaches a card that is not there yet. A fan dealt by a click arrives the same way.

The delay is cleared once it has played — `transitionend`, with a timer behind it — because focus grades every card on every click, and a card that kept its delay would inherit the wait. The release itself races a rAF pair against a 60ms timer: rAF is throttled to about once a second in a hidden tab, and the wave would otherwise sit at zero until the reader came back.
