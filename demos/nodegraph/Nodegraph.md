...
**A hop is not a column.** Every fan reaching the same distance put twelve cards in a 180px band and the hop read as a wall. Three things spread it, all deterministic so the graph is the same every session: the BOW (a fan's ends set back 160px from its middle), the RANK (enrichment sits 150px further out than the spine, so how far right a card is says how well it is attached), and a per-card drift hashed from the id, which is the only one of the three that separates five rank-1 siblings from each other. `BUILD_COL` is 820 and the spring's rest length 620; measured, the two-hop opening spans 1,645px with the far hop spread over 558 of it.
